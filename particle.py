...
def get_conv_params(L_large, L_small):
    """Return (enc_padding, dec_stride) for a Conv1D encode / ConvTranspose1D decode pair."""
    enc_padding = (L_small - 1) // 2
    dec_stride  = (L_large - L_small) // (L_small - 1)
    return enc_padding, dec_stride


# Main function
def main(seed=42, run_name='alife_journal', fixed_weights=False, max_iter_arg=1000, preset_name='13-7-3'):
    """
    Main function
    ...
    save_dir = os.path.join('saved_particles', run_name)
    os.makedirs(save_dir, exist_ok=True)
    ...
    x13_length, x7_length, x3_length = PRESETS[preset_name]
    enc_padding_1, dec_stride_1 = get_conv_params(x13_length, x7_length)
    enc_padding_2, dec_stride_2 = get_conv_params(x7_length,  x3_length)
    ...
    print(f"Reaction flags: react_13_7={react_13_7}, react_7_13={react_7_13}, react_7_3={react_7_3}, react_3_7={react_3_7}")
    ...
    # Catalytic transformation models (M13 <-> M7 <-> M3)
    model13_7 = Conv1DNet(output_channels=num_particles, kernel_size=x13_length, padding=enc_padding_1).to(device)
    model7_13 = ConvTranspose1DNet(output_channels=num_particles, kernel_size=x7_length, padding=0, stride=dec_stride_1).to(device)
    model7_3 = Conv1DNet(output_channels=num_particles, kernel_size=x7_length, padding=enc_padding_2).to(device)
    model3_7 = ConvTranspose1DNet(output_channels=num_particles, kernel_size=x3_length, padding=0, stride=dec_stride_2).to(device)
    ...
    for step in range(max_iter):
        ...
        with torch.no_grad():  # Disable gradient computation to reduce memory usage
            ...
            # Calculate reconstruction error (only for available model pairs)
                errors_dict = {}
                key_13_7  = f'{x13_length}->{x7_length}->{x13_length}'
                key_7_3   = f'{x7_length}->{x3_length}->{x7_length}'
                key_full  = f'{x13_length}->{x7_length}->{x3_length}->{x7_length}->{x13_length}'
                if react_13_7 and react_7_13:
                    error_13_and_7 = compute_average_reconstruction_error(x13, model13_7, model7_13)
                    errors_dict[key_13_7] = error_13_and_7
                    print(f"error_13_and_7: {error_13_and_7}")
                if react_7_3 and react_3_7:
                    error_7_and_3 = compute_average_reconstruction_error(x7, model7_3, model3_7)
                    errors_dict[key_7_3] = error_7_and_3
                    print(f"error_7_and_3: {error_7_and_3}")
                if react_13_7 and react_7_3 and react_3_7 and react_7_13:
                    error_13_and_3, error_13_and_7_2 = compute_average_reconstruction_error2(x13, model13_7, model7_3, model3_7, model7_13)
                    errors_dict[key_full] = error_13_and_3
...
if __name__ == "__main__":
    ...
    parser.add_argument('--run_name', type=str, default='alife_journal', help='Run name used as output subdirectory (default: alife_journal)')
    parser.add_argument('--fixed_weights', action='store_true', help='Control: fix model weights at initialization (do not update from particle states)')
    parser.add_argument('--max_iter', type=int, default=1000, help='Number of simulation steps (default: 1000)')
    parser.add_argument('--preset', type=str, default='13-7-3',
                        choices=list(PRESETS.keys()),
                        help='Molecule length preset (default: 13-7-3)')
    args = parser.parse_args()
    main(seed=args.seed, run_name=args.run_name,
